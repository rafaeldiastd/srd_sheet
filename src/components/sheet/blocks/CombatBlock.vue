...
const stats = () => [
  ...
  {
    label: 'Distr.', value: props.modStr(props.rangedAtk), icon: Wind,
...
<template>
  <div class="rounded-xl border border-border bg-card/80 p-4">
    <div class="flex items-center gap-2 mb-3">
      ...
      <span class="text-xs font-bold uppercase tracking-widest text-muted-foreground">Combate</span>
    </div>
    ...
    <div class="grid grid-cols-4 sm:grid-cols-7 gap-2 mb-3 items-stretch">
      <div
        ...
        class="rounded-lg border border-border bg-muted/60 p-2 text-center transition-all duration-200 select-none flex flex-col items-center justify-center"
        :class="stat.roll ? 'cursor-pointer hover:border-primary/50 hover:bg-primary/5 active:scale-95' : ''"
        @click="stat.roll?.()"
      >
        <component :is="stat.icon" class="w-3.5 h-3.5 mx-auto mb-1 text-muted-foreground" />
        <div class="text-[10px] text-muted-foreground uppercase tracking-wider mb-0.5">{{ stat.label }}</div>
        <div class="text-lg font-extrabold font-serif text-foreground">{{ stat.value }}</div>
        <div v-if="stat.sub" class="text-[9px] text-muted-foreground/50 mt-0.5">{{ stat.sub }}</div>
      </div>
    </div>
    ...
    <div v-if="!hideSaves" class="grid grid-cols-3 gap-2 items-stretch">
      <button @click="onRoll('Fortitude', '1d20 + @fort')"
        class="rounded-lg bg-muted/60 border border-border px-2 py-1.5 hover:border-primary/40 hover:bg-primary/5 transition-colors flex flex-col items-center justify-center">
